...
import CreateAddressCard from '../Address/CreateAddressCard'
...
export default function ContactDetail() {
  ...
  async function handleDeleteAddress(addressId) {
    ...
    if (response.status === 200) {
      await alertSucces('Address deleted succesfully')
  ...
  useEffectOnce(() => {
    ...
    fetchAddresses().then(() => console.log('Address fetched succesfully'))
  })
  return (
    <>
      <div>
          ...
          <div className="p-8">
            ...
            <div className="space-y-5 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <InfoItem icon="fas fa-user-tag" label="First Name" value={contact.first_name} />
                <InfoItem icon="fas fa-user-tag" label="Last Name" value={contact.last_name} />
              </div>
              <InfoItem icon="fas fa-envelope" label="Email" value={contact.email} />
              <InfoItem icon="fas fa-phone" label="Phone" value={contact.phone} />
            </div>
            {/* Addresses Section */}
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {/* Add Address Card */}
                <CreateAddressCard contactId={id} />

                {/* Address Card 1 */}
                {addresses.map((address) => (
                  <AddressCard key={address.id} address={address} contactId={id} onDelete={handleDeleteAddress} />
                ))}
              </div>
            </div>
            {/* Action Buttons */}
            <div className="flex justify-end space-x-4">
              <Link to="/dashboard/contacts">
                <Button variant="gray" icon="fas fa-arrow-left" size="lg">
                  Back
                </Button>
              </Link>
              <Link to={`/dashboard/contacts/${id}/edit`}>
                <Button variant="gradient" icon="fas fa-user-edit" size="lg">
                  Edit Contact
                </Button>
              </Link>
            </div>
