...
import { contactcList, contactDelete } from '../../lib/api/ContactApi'
...
export default function ContactList() {
  ...
  async function fetchContacts() {
    const response = await contactcList(token, { name, phone, email, page })
  ...
  async function handleContactDelete(id) {
    const isConfirmed = await alertConfirm('Are you sure, you want to delete this contact?')
    if (!isConfirmed) {
      return
    }
    ...
    if (response.status === 200) {
      await alertSucces('Contact deleted succesfully')
      setReload(!reload)
    } else {
      await alertError(responseBody.errors)
    }
  ...
  useEffect(() => {
    fetchContacts().then(() => console.log('Console fetched'))
  }, [reload])
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {contacts.map((contact) => (
            <ContactCard key={contact.id} contact={contact} onDelete={handleContactDelete} />
          ))}
        </div>
        {/* Pagination */}
