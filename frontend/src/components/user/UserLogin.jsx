...
export default function UserLogin() {
  ...
  // menggunakan "_" ketika hanya membuthkan 'setToken saja'

  async function handleSubmit(e) {
    e.preventDefault()

    const response = await userLogin({ username, password })
    console.info('cek response login :', response)
  ...
  return (
    <div className="animate-fade-in bg-gray-800 bg-opacity-80 p-8 rounded-xl shadow-custom border border-gray-700 backdrop-blur-sm w-full max-w-md">
      ...
      <form onSubmit={handleSubmit}>
        <Input label="Username" icon="fas fa-user" type="text" id="username" name="username" placeholder="Enter your name" value={username} onChange={(e) => setUsername(e.target.value)} required />
        <Input label="Password" icon="fas fa-lock" type="password" id="password" name="password" placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} />

        <div className="mb-6">
          <Button type="submit" variant="gradient" size="lg" icon="fas fa-sign-in-alt" className="w-full">
            Sign In
          </Button>
        </div>
